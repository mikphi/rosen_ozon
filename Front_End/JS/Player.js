...
Player.functions = {
    ...
    renderPlayerInfo: function()
    {
        ...
        var persons = _.filter(Map.items, function (item) {
          return item.type === 'person' && item.playerid === player.id; });
        ...
        var vehicles = _.filter(Map.items, function (item) {
          return item.type === 'vehicle' && item.playerid == player.id;
        });
