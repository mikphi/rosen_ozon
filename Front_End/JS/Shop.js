var Shop = {
    ...
    init: function(tiletype,isvehicle)
    {
        $("#shop-buy-item").off();
         $("#shop-buy-item").on('click', function(){
           Shop.functions.buyItem(Shop.iteminfocus)
         })
        Shop.listoptions = Shop.functions.fetchListOptions(tiletype, Map.itemActive.type);
        Shop.availableitems = Shop.functions.fetchAvailableItems(tiletype,Map.itemActive.type);
        Shop.contentitems = _.findWhere(Shop.temps.contents, { tiletype: tiletype,itemtype: Map.itemActive.type });
        Shop.propertiestorender = Shop.functions.fetchListProperties(tiletype,Map.itemActive.type);
        Shop.columns = Shop.functions.fetchListColumns(tiletype,Map.itemActive.type);
...
Shop.functions = {
    ...
    buyItem: function(item){
      ...
      if(Player.functions.moneyTransaction(item.value))
      {
        ...
        if(item.itemtype =="weapon")
        {
          ...
          Person.functions.displayMenu(Map.itemActive);
        }
    ...
    fetchListColumns:function(tiletype)
    {
      switch(tiletype)
      {
          case "vehiclestore":
    ...
    fetchAvailableItems: function(tiletype)
    {
        switch(tiletype)
        {
            case "vehiclestore":
    ...
    fetchListOptions: function(tiletype)
    {
        switch(tiletype)
        {
            case "vehiclestore":
          ...
          case "gasstation":
            if(Map.itemActive.type == "vehicle")
            {
              return Shop.Tempfuellistsettings;
            }
    ...
    fetchListProperties: function(tiletype)
    {
        switch(tiletype)
        {
          case "vehiclestore":
            ...
            if(Map.itemActive.type == "vehicle")
            {
              return Shop.Tempfuellistproperties;
            }
...
Shop.temps = {
    ...
    contents: [{ tiletype:"vehiclestore",itemtype:"person", shoptype:"vehicle", shopname: "Big Dealys bilar o sånt", welcometext:"fuck off, jag bestämmer!", opens: "10:00", closes:"18:00", listtype:"normal" },
    { tiletype:"weaponstore",shoptype:"weapon",itemtype:"person", shopname: "Willy Johnssons vapen och järnhandel", welcometext:"Leve gunns and ammo! Vapen är livet. <3 <3", opens: "08:00", closes:"20:00", listtype:"normal" },
      { tiletype:"uoffice",shoptype:"uoffice",itemtype:"person", shopname: "Arbetsförmedlingen", welcometext:"Välkommen! Fatta så jävla kul det är o jobba!!! #jävligtkul", opens: "09:00", closes:"18:00", listtype:"normal" },
        { tiletype:"gasstation",shoptype:"gasstation", itemtype:"person",shopname: "Jiffys fina bensakedja", welcometext:"Välkommen till Jiffys! ", opens: "06:00", closes:"02:00",listtype:"webshop" },
          { tiletype:"gasstation",shoptype:"gasstation", itemtype:"vehicle",shopname: "Jiffys fina bensakedja", welcometext:"Välkommen till Jiffys, o kära bilägare", opens: "06:00", closes:"02:00",listtype:"normal" },
  ]
...
Shop.Tempweaponscolumns = [
  ...
  Shop.Tempuofficepersons = [
  {
    ...
    name: "Peps Perrasson",
    img: "persons/2.png",
    smallimg:"persons/2head.png",
    description: "svensson blir nojig och jonsson blir sne",
    color: "",
    persontype: "drugdealer",
    dailycost:10,
    energy: 100,
  ...
  { tileid: 35, itemid: 1292,  tileid:70,type:"person",name: "Raven", img: "persons/1.png", smallimg: "persons/1head.png", description: "Life is pain, and so is unemployment", color: "", persontype: "gothare", dailycost: 10, attributes: {strenght:6,mobility:8,intelligence:3,leadership:4, weaponhandling:9, influence:10, currage:9, drugtolerance:8, workmoral:10, alcoholtolerant:10},energytomove: 20 },
  { tileid: 35, itemid: 122, tileid:103, type:"person",name: "Felix Reinfeldt", img: "persons/1.png", smallimg: "persons/1head.png", description: "privatisera arbetsförmedlingen.", color: "", persontype: "adeln", dailycost: 10, attributes: {strenght:10,mobility:5,intelligence:1,leadership:1, weaponhandling:4, influence:1, currage:10, drugtolerance:4, workmoral:2, alcoholtolerant:2},energytomove: 20  }
  ];
...
Shop.Tempuofficelistproperties =["smallimg", "name", "description", "persontype", "dailycost"];
...
Shop.Tempfuelitems = [
  { name: "Halv tank",description:"halv tank.... inte fy skam.", img:"fueltank.png"},
  { name: "Hel tank",description:"boss.....", img:"fueltank.png"}

];

Shop.Tempfuellistproperties = ["img","name", "description"];

Shop.Tempfuelitemscolumns= [
    { name: "", width: "10%" },
    { name: "", width: "30%" },
      { name: "", width: "60%" }
    ];

Shop.Tempfuellistsettings = { islinkable: true,
          displayheader: true,
          displayheader: false, imgwidth: "20px", imgheight:"30px",
          appendelement: "#shop-item-list",
          linkfunction: Shop.functions.renderFocusItem,
          oddcolor:"",
          evencolor:"#D6D6D6" };
